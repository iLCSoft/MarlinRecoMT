...
namespace marlinreco_mt {
  ...
  public:
     ...
     */
    virtual LCVector3D getPosition(double s, LCErrorMatrix<3>* errors=0) const = 0;
    
    /** Direction at path length s, i.e. (dx/ds,dy/ds,dz/ds) 
     *  @param s      path length
     *  @param errors 3x3 matrix, return argument - not computed if NULL
     */
    virtual LCVector3D getDirection(double s,  LCErrorMatrix<3>* errors=0) const = 0;
    
    /** Full covariance Matrix of x,y,z,px,py,pz   
     *  @param s      path length
     */
    virtual LCErrorMatrix<6> getCovarianceMatrix( double s) const = 0;
  ...
  class PhysicalTrajectory : public LCTrajectory {
    ...
    virtual ~PhysicalTrajectory() = default ;
  };
